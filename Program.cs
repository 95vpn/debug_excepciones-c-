...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    if (changeNeeded > 0)
        throw new InvalidOperationException("InvalidOperationEcxeption: the till is unable to make the correct change");
...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    int availableTents = cashTill[2] + tens;
    ...
    while ((changeNeeded > 9) && (availableTents > 0))
    {
        cashTill[2]--;
        changeNeeded -= 10;
        Console.WriteLine("\t A ten");
